...
define([
    ...
    "dojo/domReady!"
            ...
            template,on,esriRequest,registry,RasterFunction,Palette,Tooltip,SimpleLineSymbol,Graphic,Draw,Polygon) {

  return declare([BaseWidget, _WidgetsInTemplateMixin], {
    templateString: template,
    name: 'Scatterplot',
    baseClass: 'jimu-widget-Scatterplot',
    //public 
    myColor: {
      colors: {
      ...
      if(this.layer.pixelType==="U8"){
        ...
        histogramRequest.then(lang.hitch(this,function(result){
          ...
          if(this.layer.minValues.length&&this.layer.maxValues.length){
            ...
            this.yMinLabel = 0;
            this.yMaxLabel = 1;
//            this.xMinLabel = this.xmin;
//            this.xMaxLabel = this.xmax;
...
//            this.yMaxLabel = this.ymax;
    ...
    initialiseHandlers: function(){
      ...
      this.bandChangeHandler1 = on(this.xSelect,"change",lang.hitch(this,function(val){
        ...
        if(val===this.ySelect.get('value')){
          this.mapClickEnabled = false;
          domStyle.set('band-error','display','block');
        }
        else if(this.createLayerFlag)
          this.createRasterLayer();
      }));
      this.bandChangeHandler2 = on(this.ySelect,"change",lang.hitch(this,function(val){
        this.mapClickEnabled = true;
        domStyle.set('band-error','display','none');
        if(val===this.xSelect.get('value')){
          this.mapClickEnabled = false;
          domStyle.set('band-error','display','block');
        }
        else if(this.createLayerFlag)
      ...
      this.mapUpdateHandler = this.map.on("update-end",lang.hitch(this,function(){
        ...
        for(var d=this.map.layerIds.length-1;d>=0;d--){
          if(this.map.getLayer(this.map.layerIds[d]).type === "ArcGISImageServiceLayer" && this.map.layerIds[d] !== "resultLayer"){
      ...
      this.layerAddHandler = this.map.on("layer-add-result",lang.hitch(this,function(result){
        if(result.layer.id==="resultLayer")
    ...
    startup: function(){
      ...
      //create marker
      domConstruct.place('<img id="marker" style="display:none;position: absolute;z-index:100;" src="widgets/Scatterplot/images/marker.png">', "canvasBlock");
    ...
    onOpen: function(){
      ...
      if(this.supportFlag){
        ...
        if(this.map.layerIds.length>0){
          if(this.map.getLayer(this.map.layerIds[this.map.layerIds.length-1]).type==='ArcGISImageServiceLayer'){
            this.layer = this.map.getLayer(this.map.layerIds[this.map.layerIds.length-1]);
            this._populateDropDown();
          }
        }
      }
