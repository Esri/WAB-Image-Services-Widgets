...
define([
    ...
    "dijit/layout/BorderContainer"
        ...
        function(
                ...
                dom,
                domConstruct,
                domStyle, esriRequest, ImageServiceIdentifyTask, ImageServiceIdentifyParameters, Point, bundle, Chart, Tooltip, theme, SelectableLegend, Magnify, locale,Graphic, SimpleMarkerSymbol,SimpleLineSymbol,Color, Draw, RasterFunction) {
            var clazz = declare([BaseWidget, _WidgetsInTemplateMixin], {
                  ...
                  if (this.map.layerIds) {
                    ...
                    if(this.primaryLayer !== this.prevprimaryLayer){
                      if(this.layerInfos[this.label]){
                        ...
                        var options=[{"label":"Current Image","value":"nonTemporal"}];
                        if(this.layerInfos[this.label].hasOverlap)
                          options.push({"label":"Overlapping Images","value":"temporal"});
                        if(this.layerInfos[this.label].hasNDVI)
                          options.push({"label":"Indices (NDVI)","value":"NDVI"});
                      ...
                      if(!this.layerInfos[this.label]){
                        ...
                        if(this.primaryLayer.type==="ArcGISImageServiceLayer"){
                          ...
                          if(this.layerObj.acquisitionDate&&this.layerObj.category){
                ...
                spectralprofile : function(point){
                  ...
                  imageTask.execute(imageParams, lang.hitch(this, function(data) {
                    if(data.properties.Values.length){
                        ...
                        if (items[a].attributes[this.layerInfos[this.label].category] === 1) {
                          ...
                          for (var j in plot) {
                            ...
                            normalizedValues.push({
                              y: calc,
                              tooltip: (locale.format(new Date(items[a].attributes[this.layerInfos[this.label].acquisitionDate]), {selector: "date", formatLength: "long"})) + ", " + calc.toFixed(2)
                            });
                          }
                        ...
                        if (items[a].attributes[this.layerInfos[this.label].category] === 1) {
                          ...
                          if(this.layerInfos[this.label].ndmi){
                            ...
                            normalizedValues1.push({
                              y: ndmi,
                              tooltip: locale.format(new Date(items[a].attributes[this.layerInfos[this.label].acquisitionDate]), {selector: "date", formatLength: "long"}) + ", " + ndmi.toFixed(2)
                            });
                          ...
                          if(this.layerInfos[this.label].urban){
                            ...
                            normalizedValues2.push({
                              y: urban,
                              tooltip: locale.format(new Date(items[a].attributes[this.layerInfos[this.label].acquisitionDate]), {selector: "date", formatLength: "long"}) + ", " + urban.toFixed(2)
                            });
                    ...
                    if (registry.byId("type").get("value") === "nonTemporal") {
                      ...
                      if(this.layerInfos[this.label].bandCount>1){
                        ...
                        for (var x in this.temporalData) {
                          this.axesLabels[x] = {text: locale.format(new Date(this.temporalData[x].acqDate), {selector: "date", formatLength: "long"}),value:parseInt(x)+1};
                          this.tempData[x] = this.temporalData[x].values[0];
                        }
                        this.chart.addAxis("x", {labels: this.axesLabels, labelSizeChange: true, title: "AcquisitionDate", titleOrientation: "away", minorTicks: false, majorTickStep: 1});
